...
df3 = pd.DataFrame(index=list(df['Year'].values), columns=[
                   'Total Ridership (in millions)'])
df3['Total Ridership (in millions)'] = list(
    df['All Modes Reported Total (Parts A and B)'].values)
df3.index.name='Year'
df_2.set_index('Year')
df3.to_csv('ridership_US.csv')
