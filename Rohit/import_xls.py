# %%
# Author - Rohit Kumar - rokumar@ucsd.edu
# Github - rohit1347
...
class apta_utils():
    def __init__(self):
        self.cities = {'CA': ['San Diego', 'San Francisco', 'Los Angeles'], 'WA': ['Seattle'], 'TX': ['Austin', 'Houston', 'Dallas'], 'NY': ['New York'], 'IL': ['Chicago'], 'MI': ['Detroit'],
        ...
        self.bokeh_counties = {'CA': ['San Diego County, California', 'San Francisco County, California', 'Los Angeles County, California'], 'WA': ['King County, Washington'], 'TX': ['Austin County, Texas', 'Houston County, Texas', 'Dallas County, Texas'], 'NY': ['New York County, New York'], 'IL': ['Cook County, Illinois'], 'MI': ['Wayne County, Michigan'],
        ...
        self.col_index = [1, 2, 4, 6, 8, 10, 12]
        # Column indices for US county data
        self.col_index2 = [1, 3, 5, 7, 9, 11, 13]
        # Column indices for old US county data
        self.col_index_names = ['Urban Population', 'Vehicles in Service', 'Vehicles Available',
                                'Vehicle Revenue Miles', 'Vehicle Revenue Hours', 'Unlinked Passenger Trips', 'Passenger Miles']
        self.col_index_names1000 = ['Urban Population', 'Vehicles in Service (per 1000 persons)', 'Vehicles Available (per 1000 persons)',
        ...
        self.col_index_names_p = ['Urban Population', 'Vehicles in Service (per person)', 'Vehicles Available (per person)',
    ...
    def get_xls(self, pflag=0):
        ...
        for year in datasets:
            years.append(
                int(''.join([y for y in list(year) if y.isdigit()]))-2)
    ...
    def create_empty_city_dataframes(self, pflag=0):
        """Returns a dictionary containing dataframes corresponding to cities.

        Keyword Arguments:
            pflag {int} -- Print flag, use to check empty dataframes dict(default: {0})
        """
        cities = self.cities
        ...
        for state in cities.keys():
            ...
            for cix, city in enumerate(cities[state]):
                dataframe_dict[state][cix] = pd.DataFrame(
                    columns=self.col_index_names, index=self.get_xls()[-1])
                dataframe_dict[state][cix].index.name = 'Year'
        if pflag:
            print('Created empty dataframes dict')
        return dataframe_dict

    def get_city_indices(self, sheet=pd.DataFrame()):
    ...
    def create_city_dataframes(self, pflag=0):
        ...
        datasets, years = self.get_xls()
        filled_frames = self.create_empty_city_dataframes()
        for di, excel in enumerate(datasets):
            ...
            cindexer = self.get_city_indices(sheet=sheet)
            for state in filled_frames.keys():
                for ix, city_df in enumerate(filled_frames[state]):
                    ci = self.col_index2 if len(
                        cnames) == 15 else self.col_index
                    data = []
    ...
    def get_simple_plots(self, filled_frames, state='CA', city_index=0):
    ...
    def transform_city_dataframes(self, filled_frames, ttype=[0]):
            ...
            if ty == 0:
                ...
                for state in mdf.keys():
                    for city, df in enumerate(mdf[state]):
                        df.columns = self.col_index_names1000
                        df.iloc[:, 1:] = df.iloc[:, 1:].div(1/1000).div(
                            df.iloc[:, 0], axis='index')
            if ty == 1:
                for state in mdf.keys():
                    for city, df in enumerate(mdf[state]):
                        df.columns = self.col_index_names_p
    ...
    def get_sales_data(self, fname='TOTALSA.csv'):
        ...
        """
        assert os.path.exists(os.path.abspath(fname))
    ...
    def interpolate_dataframes(self, ff):
    ...
    def get_car_economy(self, fname='car_economy.csv'):
    ...
    def get_us_ridership(self, fname='ridership_US.csv'):
    ...
    def combine_for_correlation(self, df1=self.get_us_ridership(), df2=self.get_sales_data()):
    ...
    def create_correlation_plot(self, df):
    ...
    def get_mv_deaths(self, fname='deaths-and-population-ra.csv'):
    ...
    def get_fuel_usage(self, fname='2019-APTA-Fact-Book-Appendix-A.xlsx'):
        """Returns the diesel consumed by public transport over the years.

        Keyword Arguments:
            fname {str} -- File from which to extract data (default: {'2019-APTA-Fact-Book-Appendix-A.xlsx'})
        """
        assert os.path.exists(os.path.abspath(fname))
        ...
        fc['Diesel Usage'] = fc['Diesel Usage'].str.extract(
            '(\d+)', expand=False)
        fc['Diesel Usage'] = fc['Diesel Usage'].astype(int)
        fc['Diesel Usage'] = fc['Diesel Usage'] * 1e6
        return fc

    def get_bus_miles(self, fname='2019-APTA-Fact-Book-Appendix-A.xlsx'):
        """Returns the revenue miles covered by public transport over the years.
    ...
    def create_fuelsaved_plot(self):
        """
        Creates a plot of the fuel saved using PT miles and fuel data, and average mpg of personal vehicles over the years.
        """
        temp1 = self.combine_for_correlation(
            df1=self.get_fuel_usage(), df2=self.get_bus_miles())
        temp2 = self.combine_for_correlation(
            df1=temp1, df2=self.get_car_economy())
        ...
        ax.plot(temp2['Car Fuel Usage'],
                label='Car Fuel Usage', color='#80D9FF')
...
# %%
start = time.time()
tp = u.create_city_dataframes()
end = time.time()
print(f'Time to compute dataframes: {end-start:.2f}')
# %%
tp = u.interpolate_dataframes(tp)
# %%
sd = tp["CA"][0]

# %%
h = next(u.transform_city_dataframes(tp, ttype=[0]))

# %% Plotting
u.get_simple_plots(h, state='NY')

# %% Bokeh Plotting
u.create_bokeh_choro(h, prop=5)
